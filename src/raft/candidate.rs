...
impl RaftProtocol for RaftCandidateStateDelegate {
    ...
    fn time_step(&mut self, shared_state: &mut SharedState) -> (RaftMessageStateChange, u64) {
                ...
                PersistenceResponseType::VotePersisted {
                    ...
                    if let Some(mut msg) = shared_state.outstanding_messages.remove(&id) {
                        ...
                        match msg.message_type {
                            ...
                            OutstandingMessageType::CandidateElection {
                                persistence_done: _,
                                quorum_votes,
                            } => {
                                if quorum_votes >= shared_state.quorum_size {
                                    ...
                                    return (
                                        RaftMessageStateChange::Leader(Box::new(RaftLeaderStateDelegate::new())),
                                        work_done,
                                    );
                                } else {
                                    msg.outstanding_responses = new_outstanding_resp;
                                    // TODO maybe not recreate this all of the time.
                                    let new_state = OutstandingMessageType::CandidateElection {
            ...
            match task.response_type {
                ...
                LocalQuorumTaskResponseType::RequestVoteResponse {
                    ...
                    if let Some(mut msg) = shared_state.outstanding_messages.remove(&id) {
                        ...
                        log::error!(
                            "Received quorum event RequestVoteResponse with no outstanding message registered {}",
                            id
                        );
                    }
                }
        ...
        if timeout_expired {
            ...
            log::warn!(
                "Initiating election due to timeout {}, now {}, id {}",
                self.initiate_election_timeout,
                now,
                message_id
            );
            ...
            if shared_state.term_votes.get(&next_term).is_some() {
                log::error!(
                    "trying to start an election for a term it has already voted {}",
                    next_term
                )
            }
            shared_state.volatile_server_state.next_term = next_term + 1;
            let new_election = OngoingElection {
                term: next_term,
                started_millis: now,
            };
            ...
            shared_state.quorum_worker_tasks.iter().for_each(|task_queue| {
                ...
                let task = LocalQuorumWorkerTaskType::RequestVote {
                    id: message_id,
                    term: next_term,
                    last_term,
                    last_index,
            ...
            let persistence_task = PersistenceTaskType::AppendVote {
                id: message_id,
                term: next_term,
                candidate_id: shared_state.identity,
                parent_span: persistence_span,
            };
            ...
            let message_type = OutstandingMessageType::CandidateElection {
            ...
            self.initiate_election_timeout =
                now_plus_duration_millis(Duration::from_millis(self.rng.gen_range(500..3000) as u64));
        }

...
use crate::transport::capnp::{
    build_owned_write_batch_response, OwnedLogEntry, OwnedWriteBatch, OwnedWriteBatchResponse,
};
use bus::Bus;
...
pub enum OutstandingMessageType {
    ...
    }, // Request vote from someone else
    CandidateElection {
        persistence_done: bool,
        quorum_votes: u32,
    }, // Candidate election by this node.
    AppendLog {
        callback: oneshot::Sender<LocalRaftResponseMessage>,
    },
    WriteBatch {
...
impl SharedState {
        ...
        for i in start_index..=new_commit_index {
            ...
            if let Some(entry) = self.volatile_server_state.replication_log.get(idx) {
                ...
                match self.log_entry_bus.try_broadcast(e) {
                    ...
                    Err(_) => {
                        last_broadcast_index =
                            self.broadcast_committed_entries_when_bus_full(new_commit_index as usize, idx) as u64;
                        break;
                    }
                }
            } else {
                warn!(
                    "Didnt find index {} in replication log of length {}",
                    idx,
                    self.volatile_server_state.replication_log.len()
                )
            }
        }
    ...
    fn broadcast_committed_entries_when_bus_full(
        &mut self,
        new_absolute_commit_index: usize,
        start_index: usize,
    ) -> usize {
        ...
        match raft_message.payload {
            LocalRaftMessagePayload::RequestVote(r) => {
                ...
                let state_change = self
                    .state_delegate
                    .request_vote(r, raft_message.callback, &mut self.shared_state);
                match state_change {
                    RaftMessageStateChange::None => {}
                    RaftMessageStateChange::Candidate(new_state) => self.state_delegate = new_state,
                    RaftMessageStateChange::Follower(new_state) => self.state_delegate = new_state,
                    RaftMessageStateChange::Leader(new_state) => self.state_delegate = new_state,
                }
            }
            LocalRaftMessagePayload::AppendEntries(r) => {
                ...
                let state_change = self
                    .state_delegate
                    .append_entries(r, raft_message.callback, &mut self.shared_state);
            ...
            LocalRaftMessagePayload::GetRaftState => {
                ...
                span.set_parent(raft_message.parent_span.context());
                span.in_scope(|| {
                    let resp_payload = LocalRaftResponsePayload::RaftState {
                        leader_id: self.shared_state.server_state.leader_id,
                    };
                    let resp = LocalRaftResponseMessage { payload: resp_payload };
                    raft_message
                        .callback
                        .send(resp)
                        .unwrap_or_else(|_| log::warn!("Failed to send raft callback: receiver dropped"))
                })
            }
            LocalRaftMessagePayload::WriteBatch(batch) => {
                ...
                if self.state_delegate.get_node_type() == RaftNodeType::Leader {
                    ...
                    self.state_delegate
                        .write_batch(batch, raft_message.callback, &mut self.shared_state)
                } else {
                    log::error!(
                        "Received write batch as non-leader. Leader is {}",
                        self.shared_state.server_state.leader_id
                    );
                    ...
                    raft_message
                        .callback
                        .send(LocalRaftResponseMessage { payload: msg })
                        .unwrap_or_else(|_| log::warn!("Failed to send raft callback: receiver dropped"))
                }
            }
        }
...
pub trait RaftProtocol {
    ...
    fn should_accept_vote(
        ...
        if let Some(voted_for) = shared_state.term_votes.get(&term) {
            ...
            info!(
                "Rejecting vote, node has already voted for candidate {} this term {}",
                voted_for, term
            );
            return false;
        }
        ...
        if term <= shared_state.server_state.current_term {
            info!(
                "Rejecting vote, proposed term {} is lower than current {}.",
...
impl std::fmt::Debug for LocalRaftWriteBatchResponse {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "LocalRaftWriteBatchResponse {{ message: {:?}, err: {:?} }}",

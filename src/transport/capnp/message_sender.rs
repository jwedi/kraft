use super::owned_message::{build_owned_message, OwnedQuorumMessage};
...
impl MessageSender {
    pub fn new(node_id: u32, remote_addr: SocketAddr, rx: CrossbeamReceiver<OwnedQuorumMessage>) -> Self {
        Self {
            node_id,
            remote_addr,
            rx,
        ...
        let reader_handle = tokio::spawn(async move {
            loop {
                match read_message(&mut reader).await {
                    ...
                    Err(e) => {
                        log::warn!(
                            "Sender receive stream closed remote connection to {}: {}",
...
pub async fn write_message<W: AsyncWriteExt + Unpin>(writer: &mut W, msg: &OwnedQuorumMessage) -> std::io::Result<()> {
...
pub async fn read_message<R: AsyncReadExt + Unpin>(reader: &mut R) -> std::io::Result<OwnedQuorumMessage> {
    ...
    OwnedQuorumMessage::from_bytes(bytes).map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e))
}
